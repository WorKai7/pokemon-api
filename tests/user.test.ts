...
describe('User API', () => {
  describe('POST /users', () => {
    it('should create a new user', async () => {
      ...
      const response = await request(app).post("/users").set("Authorization", `Bearer ${token}`).send(createdUser);
  ...
  describe("GET /users/:userId", () => {
    it("should get a user", async () => {
      const mockUser = { id: 2, email: "mail2", password: "gr5fGR2jnCs4V" };
  ...
  describe("PATCH /users/:userId", () => {
    it("should update a user", async () => {
      ...
      const updatedUser = { id: 1, email: "updatedemail", password: "updatedpass" };
      
      prismaMock.user.findUnique.mockResolvedValue(userToUpdate);
      prismaMock.user.update.mockResolvedValue(updatedUser);
      
      const response = await request(app).patch("/users/:userId").set("Authorization", `Bearer ${token}`).send(updatedUser);
    ...
    it("should return an error (user not found)", async () => {
      ...
      const response = await request(app).patch("/users/:userId").set("Authorization", `Bearer ${token}`);
