...
export const getPokemonAttack = async (req: Request, res: Response) => {
    try {
        const pokemonAttack = await prisma.pokemonAttack.findUnique({
            where: { id: +req.params.pokemonAttackId },
        });
...
export const createPokemonAttack = async (req: Request, res: Response) => {
    try {
        ...
        const doublon = await prisma.pokemonAttack.findFirst({
            where: {
                AND: [
                    { name: req.body.name },
...
export const updatePokemonAttack = async (req: Request, res: Response) => {
    try {
        ...
        const updateAttack = await prisma.pokemonAttack.findUnique({
            where: { id: +req.params.pokemonAttackId },
        });
        if (!updateAttack) {
        ...
        await prisma.pokemonAttack.update({
            where: { id: +req.params.pokemonAttackId },
            data: req.body,
        });
...
export const deletePokemonAttack = async (req: Request, res: Response) => {
    try {
        const deleteAttack = await prisma.pokemonAttack.findUnique({
            where: { id: +req.params.pokemonAttackId },
        });
        if (!deleteAttack) {
            res.status(404).json({ error: "Attack not found" });
