...
export const getPokemon = async (req: Request, res: Response) => {
    try {
        ...
        const pokemons = await prisma.pokemonCard.findUnique({ where: { id: +req.params.pokemonCardId } });
...
export const createPokemon = async (req: Request, res: Response) => {
    try {
        ...
        const attackExists = await prisma.pokemonAttack.findUnique({ where: { id: req.body.attackId } });
        if (!attackExists) {
        ...
        if (req.body.weaknessId) {
            const weaknessExists = await prisma.type.findUnique({ where: { id: req.body.weaknessId } });
            if (!weaknessExists) {
        ...
        const doublon = await prisma.pokemonCard.findFirst({ where: { OR: [{ name: req.body.name }, { pokedexId: req.body.pokedexId }] } });
        if (doublon) {
...
export const updatePokemon = async (req: Request, res: Response) => {
    try {
        ...
        const updatePokemon = await prisma.pokemonCard.findUnique({ where: { id: +req.params.pokemonCardId } });
        if (!updatePokemon) {
        ...
        await prisma.pokemonCard.update({ where: { id: +req.params.pokemonCardId }, data: req.body });
...
export const deletePokemon = async (req: Request, res: Response) => {
    try {
        ...
        const deletePokemon = await prisma.pokemonCard.findUnique({ where: { id: +req.params.pokemonCardId } });
        if (!deletePokemon) {
            res.status(404).json({ error: "Pokemon not found" })
            return;
        }

        await prisma.pokemonCard.delete({ where: { id: +req.params.pokemonCardId } });
        res.status(204).send(`Le pokémon n°${req.params.pokemonCardId} à été supprimé de la base de données avec succès !`);
